...
//			System.out.println("PRESSED keychar='" + e.getKeyChar() + "' keycode=0x"
...
//			if(e.getY() != 0) 
	...
	public boolean handleShortcutKeys(long time, KeyEvent e, boolean pressed) {
		...
		case KeyEvent.VK_ENTER:
            ...
            altDown = false;
            //@deprecated ((RdesktopJPanel_Localised) canvas.getParent()).toggleFullScreen();
			break;
	...
	{
		sendScancode(getTime(), RDP_KEYRELEASE, key);	// CTRL
	}
	
	/**
	 * Send CTRL-Shift combination.
	 */
	public void sendCtrlShift()
	{
		sendScancode(getTime(), RDP_KEYPRESS, 0x1d);	// CTRL
		sendScancode(getTime(), RDP_KEYPRESS, 0x2a);	// SHIFT
		sendScancode(getTime(), RDP_KEYRELEASE, 0x2a);	// SHIFT
	...
	/**
	 * Send CTRL-Blank combination
	 */
	public void sendCtrlBlank()
	{
		sendScancode(getTime(), RDP_KEYPRESS, 0x1d);
		sendScancode(getTime(), RDP_KEYPRESS, 0x39);
		sendScancode(getTime(), RDP_KEYRELEASE, 0x39);
		sendScancode(getTime(), RDP_KEYRELEASE, 0x1d);
		
	}
	/**
	 * Send One by split info
	 */
	public void sendOneByKeysRdp5()
	{		//"*	"	"
		if(InputJPanel.key_Code)
		{
			sendKeyPresses("78");//*	 
			InputJPanel.key_Code=false;
		}
		else
		{
			sendKeyPresses("58");//*	
			InputJPanel.key_Code=true;
		}
//		Integer keycode = new Integer(106);
//		pressedKeys.removeElement(keycode);
//		sendKeyPresses("");
	}
    /**
